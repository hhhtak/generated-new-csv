...
export interface DataTransformer {
  ...
  /**
   * Add sequence number columns with auto-incrementing values
   */
  addSequenceColumns(
    data: CSVData,
    sequenceColumns: Array<{ column: string; start?: number; step?: number }>,
  ): CSVData;
}
...
export class DataTransformerImpl implements DataTransformer {
  ...
  transform(data: CSVData, config: TransformationConfig): CSVData {
    ...
    try {
      ...
      // Add sequence columns before column reordering
      if (config.sequenceColumns && config.sequenceColumns.length > 0) {
        this.logger.debug("Adding sequence columns", {
          columns: config.sequenceColumns.length,
        });
        transformedData = this.addSequenceColumns(
          transformedData,
  ...
  /**
   * Add sequence number columns with auto-incrementing values
   * Validates that sequence column names don't conflict with existing columns
   */
  addSequenceColumns(
    data: CSVData,
    sequenceColumns: Array<{ column: string; start?: number; step?: number }>,
  ): CSVData {
    // Check for duplicate column names with existing columns
    const duplicateColumns = sequenceColumns
      .map((seq) => seq.column)
      .filter((columnName) => data.headers.includes(columnName));

    if (duplicateColumns.length > 0) {
      throw this.errorHandler.handleTransformationError(
        `連番列の名前が既存の列と重複しています: '${duplicateColumns.join(
          ", ",
        )}'`,
        {
          duplicateColumns,
          existingHeaders: data.headers,
          sequenceColumns: sequenceColumns.map((seq) => seq.column),
        },
      );
    }

    // Add sequence column headers
    const newHeaders = [
      ...data.headers,
      ...sequenceColumns.map((seq) => seq.column),
    ];

    // Add sequence values to each row
    const newRows = data.rows.map((row, rowIndex) => {
      const newRow = [...row];
      sequenceColumns.forEach((seq) => {
        const start = seq.start !== undefined ? seq.start : 1;
        const step = seq.step !== undefined ? seq.step : 1;
        const sequenceValue = start + rowIndex * step;
        newRow.push(String(sequenceValue));
      });
      return newRow;
    });
